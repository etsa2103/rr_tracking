...
class RosGui(QWidget):
    def __init__(self):
        ...
        self.mask_checkbox.setChecked(rospy.get_param("/rr_tracker/use_mask", True))
    ...
    def tracking_state_cb(self, msg):
        try:
            mask_enabled = rospy.get_param("/rr_tracker/use_mask", True)
    ...
    def toggle_mask(self, state):
        rospy.set_param("/rr_tracker/use_mask", state == Qt.Checked)
    ...
    def update_plot(self):
        try:
            ...
            rospy.logerr(f"[rr_tracker/gui] error in update_plot: {e}")
    ...
    def schedule_next_plot(self):
        try:
            ...
            rospy.logerr(f"[rr_tracker/gui] error: {e}")
